...
sub serialize {
	my ($obj, $file) = @_;
	my $ice = freeze(
		_can($obj, 'serial') ? $obj->serial : $obj
	);
	writefile($file, $ice) if defined $file;
	return $ice;
}
sub unserialize {
	shift if _can($_[0], 'unserialize'); # turf the obj if necessary
...
# --- replaces the deprecated UNIVERSAL::can ---
sub _can {
	my ($obj, $method) = @_;
	return undef unless blessed $obj;
...
Puma::Object::Serial - extension for object serialization

=head1 SYNOPSIS

  As an Export:
    use Puma::Object::Serial;
...
B<Puma::Object::Serial> is both a base class and function exporter for data serialization and storage.
